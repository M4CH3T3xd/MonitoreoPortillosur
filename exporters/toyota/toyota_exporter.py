...
import time
from db_utils import get_logger, ejecutar_select, ejecutar_select_all, tracer
...
def actualizar_metricas():
    ...
    with tracer.start_as_current_span("actualizar_metricas_toyota") as span_ciclo:
        span_ciclo.set_attribute("pipeline", "toyota-salesforce-sap")

        ok_salud, _, latencia = ejecutar_select(logger,
            "SELECT MAX(origen_fecha) FROM oportunidades_toyota.oportunidades_procesadas;",
            span_name="salud_bd_latencia"
        )
        if ok_salud:
            PROMETHEUS_LATENCIA.set(latencia)
            span_ciclo.set_attribute("db.latencia_ms", round(latencia, 2))

        with tracer.start_as_current_span("flujo1_lag_salesforce"):
            query_lag = """
            ...
            """
            _, res_lag, _ = ejecutar_select(logger, query_lag, span_name="query_lag_json")
            if res_lag:
                ...
                if lag_val > 0:
                    ...
                    """
                    _, det_lag, _ = ejecutar_select_all(logger, q_det_lag, span_name="query_lag_detalle")
        ...
        with tracer.start_as_current_span("flujo2_sin_nota_venta"):
            query_nv = """
                SELECT COUNT(p.id)
                ...
                WHERE nv.id IS NULL;
            """
            _, res_nv, _ = ejecutar_select(logger, query_nv, span_name="query_sin_notaventa")
            if res_nv:
                cant_nv = res_nv[0]
                ...
                if cant_nv > 0:
                    ...
                    """
                    _, det_nv, _ = ejecutar_select_all(logger, q_det_nv, span_name="query_sin_notaventa_detalle")
        ...
        with tracer.start_as_current_span("flujo3_atascados_sap"):
            query_sap = """
                SELECT COUNT(nv.id)
            ...
            """
            _, res_sap, _ = ejecutar_select(logger, query_sap, span_name="query_atascados_sap")
            if res_sap:
                cant_sap = res_sap[0]
                ...
                if cant_sap > 0:
                    ...
                    """
                    _, det_sap, _ = ejecutar_select_all(logger, q_det_sap, span_name="query_atascados_sap_detalle")
        ...
        with tracer.start_as_current_span("flujo4_pipeline_ventas"):
            query_pend = """
            ...
            """
            _, res_pend, _ = ejecutar_select(logger, query_pend, span_name="query_pendientes_totales")
            if res_pend:
                ...
                """
                _, det_pend, _ = ejecutar_select_all(logger, q_det_pend, span_name="query_pendientes_desglose")
